...
# New: Initialize number input values in session state
if 'shuttle_val' not in st.session_state:
    st.session_state.shuttle_val = 20
if 'walkin_val' not in st.session_state:
    st.session_state.walkin_val = 60
if 'court_val' not in st.session_state:
    st.session_state.court_val = 0
if 'real_shuttle_val' not in st.session_state:
    st.session_state.real_shuttle_val = 0
...
with col1:
    shuttle_val = st.number_input("ค่าลูก:", value=st.session_state.shuttle_val, key='shuttle_input', step=1)
with col2:
    walkin_val = st.number_input("ค่าคอร์ดต่อคน:", value=st.session_state.walkin_val, key='walkin_input', step=1)
with col3:
    court_val = st.number_input("ค่าเช่าคอร์ด:", value=st.session_state.court_val, key='court_input', step=1)
with col4:
    real_shuttle_val = st.number_input("ค่าลูกตามจริง:", value=st.session_state.real_shuttle_val, key='real_shuttle_input', step=1)
